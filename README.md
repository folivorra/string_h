...
В данном проекте мне предстояло разработать собственную реализацию библиотеки string.h на языке программирования С с некоторым дополнением (с собственной реализацией функций sprintf). Библиотека string.h является основной библиотекой языка С по обработке строк. В рамках этого проекта я отработал навыки взаимодействия со строковыми данными и усилил структурный подход.
...
## Build and test with HTML report by GCOV

```bash
$ make // собрать статичную библиотеку build/s21_string.a
$ make test // прогнать все тесты
$ make gcov_report // отобразить html отчет по покрытию кода тестами, необходимо подгрузить пакет gcov на систему
```